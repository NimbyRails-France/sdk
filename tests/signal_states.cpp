...
int main() {
    ...
    uint32_t count=99;
    CHECK(NimbySdk_CopySignalStates(123,nullptr,0,&count)==NIMBY_OK&&count==2);
    ...
    CHECK(NimbySdk_CopySignalStates(123,output,1,&count)==NIMBY_BUFFER_TOO_SMALL&&count==2);
    CHECK(std::memcmp(before,output,sizeof output)==0);
    CHECK(NimbySdk_CopySignalStates(123,output,2,&count)==NIMBY_OK&&count==2);
    ...
    CHECK(NimbySdk_CopySignalStates(123,output,2,&count)==NIMBY_OK&&output[1].aspect==NIMBY_SIGNAL_ASPECT_STOP);
    CHECK(NimbySdk_CopySignalStates(124,nullptr,0,&count)==NIMBY_OK&&count==0);
    CHECK(NimbySdk_CopySignalTextures(123,nullptr,0,&count)==NIMBY_OK&&count==2);
    NimbySignalTexture textures[2]{};
    CHECK(NimbySdk_CopySignalTextures(123,textures,1,&count)==NIMBY_BUFFER_TOO_SMALL&&count==2);
    CHECK(textures[0].signal_id==0);
    CHECK(NimbySdk_CopySignalTextures(123,textures,2,&count)==NIMBY_OK&&count==2);
    CHECK(std::memcmp(&textures[0],&texture,sizeof texture)==0);
    textures[0].selected_index=0;
    CHECK(NimbySdk_CopySignalTextures(123,textures,2,&count)==NIMBY_OK&&textures[0].selected_index==10);
    CHECK(NimbySdk_CopySignalTextures(124,nullptr,0,&count)==NIMBY_OK&&count==0);
    CHECK(NimbySdk_ReleaseSnapshot(123)==NIMBY_OK);
    CHECK(NimbySdk_CopySignalStates(123,output,2,&count)==NIMBY_INVALID_HANDLE&&count==0);
    CHECK(NimbySdk_CopySignalTextures(123,textures,2,&count)==NIMBY_INVALID_HANDLE&&count==0);
    CHECK(NimbySdk_ReleaseSnapshot(124)==NIMBY_OK);
