...
Télécharger **NimbyRailsSDK-0.4.0-drop-in-windows-x64.zip** dans les Assets de
https://github.com/NimbyRails-France/sdk/releases/tag/v0.4.0 et extraire tout le ZIP.
...
avec `-Action Remove`, puis installer ce paquet. Conserver le manifeste dans le dossier du jeu.

Lancer ensuite le jeu normalement depuis Steam : le SDK est chargé après SDL_Init
à chaque démarrage. Aucun chargeur externe à lancer. Ne pas lancer NimbyRailsLoader en parallèle.
Journal : `%LOCALAPPDATA%/NimbyRailsSDK/proxy.log`.
...
2. Copier **SDL3.dll**, **NimbyRailsSDK.dll** et **libwinpthread-1.dll** du paquet dans ce même dossier.
